function p = permutation_test(x, y, tail, resampling, xlabel_text)
...
for i = 1:resampling
    ...
    for j = 1:N
        ...
        ny = numel(y{j});
        xy = [x{j}(:); y{j}(:)];
        IDshuffle = randperm(nx+ny);
        xyshuffle = xy(IDshuffle);
        xmean_all(j) = mean(xyshuffle(1:nx));
        ymean_all(j) = mean(xyshuffle(nx+1:end));
    end
...
xlabel(xlabel_text);
ylabel('probability');
title(['Permutation Test: p = ' num2str(p)]);
hold off;