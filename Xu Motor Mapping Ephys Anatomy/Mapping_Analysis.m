...
PG2REF = undistortPoints([PG2X0 resolution(1)-PG2Z0], CP2);
...
%     display(delay);
    ...
    cmap = jet(size(y_all, 1)-1);
    cmap = flipud(cmap);
    figure;
    subplot(2, 2, 1);
    hold on;
    for j = 1:size(y_all, 2)
        for k = 1:size(y_all, 1)-1
            plot(y_all(k:k+1, j)-y_all(1, j), z1_all(k:k+1, j)-z1_all(1, j), '-', 'Color', cmap(k, :), 'LineWidth', 1);
        end
        plot(y_all(end, j)-y_all(1, j), z1_all(end, j)-z1_all(1, j), 's', 'Color', cmap(end, :), 'MarkerFaceColor', cmap(end, :), 'MarkerSize', markersize);
    end
    if size(y_all, 2) > 1
        plot(mean(y_all, 2)-mean(y_all(1, :)), mean(z1_all, 2)-mean(z1_all(1, :)), '-k', 'LineWidth', 3);
        plot(mean(y_all(end, :)-y_all(1, :)), mean(z1_all(end, :)-z1_all(1, :)), 'sk', 'MarkerFaceColor','k', 'MarkerSize', markersize);
    ...
    for j = 1:size(y_all, 2)
        for k = 1:size(y_all, 1)-1
            plot(y_all(k:k+1, j), z1_all(k:k+1, j), '-', 'Color', cmap(k, :), 'LineWidth', 1);
        end
        hold on;
        plot(y_all(1, j), z1_all(1, j), 'o', 'Color', cmap(1, :), 'MarkerFaceColor', cmap(1, :), 'MarkerSize', markersize);
        plot(y_all(end, j), z1_all(end, j), 's', 'Color', cmap(end, :), 'MarkerFaceColor', cmap(end, :), 'MarkerSize', markersize);
    end
    if size(y_all, 2) > 1
        plot(mean(y_all, 2), mean(z1_all, 2), '-k', 'LineWidth', 3);
        plot(mean(y_all(1, :)), mean(z1_all(1, :)), 'ok', 'MarkerFaceColor','k', 'MarkerSize', markersize);
    ...
    hold on;
    for j = 1:size(x_all, 2)
        for k = 1:size(x_all, 1)-1
            plot(x_all(k:k+1, j)-x_all(1, j), z2_all(k:k+1, j)-z2_all(1, j), '-', 'Color', cmap(k, :), 'LineWidth', 1);
        end
        plot(x_all(end, j)-x_all(1, j), z2_all(end, j)-z2_all(1, j), 's', 'Color', cmap(end, :), 'MarkerFaceColor', cmap(end, :), 'MarkerSize', markersize);
    end
    if size(x_all, 2) > 1
        plot(mean(x_all, 2)-mean(x_all(1, :)), mean(z2_all, 2)-mean(z2_all(1, :)), '-k', 'LineWidth', 3);
        plot(mean(x_all(end, :)-x_all(1, :)), mean(z2_all(end, :)-z2_all(1, :)), 'sk', 'MarkerFaceColor','k', 'MarkerSize', markersize);
    ...
    for j = 1:size(x_all, 2)
        for k = 1:size(x_all, 1)-1
            plot(x_all(k:k+1, j), z2_all(k:k+1, j), '-', 'Color', cmap(k, :), 'LineWidth', 1);
        end
        plot(x_all(1, j), z2_all(1, j), 'o', 'Color', cmap(1, :), 'MarkerFaceColor', cmap(1, :), 'MarkerSize', markersize);
        plot(x_all(end, j), z2_all(end, j), 's', 'Color', cmap(end, :), 'MarkerFaceColor', cmap(end, :), 'MarkerSize', markersize);
    end
    if size(x_all, 2) > 1
        plot(mean(x_all, 2), mean(z2_all, 2), '-k', 'LineWidth', 3);
        plot(mean(x_all(1, :)), mean(z2_all(1, :)), 'ok', 'MarkerFaceColor','k', 'MarkerSize', markersize);
    ...
    % for publication start
    avgcolor = [0 0 0];
    cmap = jet(size(x_all, 1)-1);
    cmap = flipud(cmap);
    figure;
    hold on;
    for j = 1:realrepeats
        for k = 1:size(x_all, 1)-1
            plot3(x_all(k:k+1, PG2repeat_all == commonrepeat(j)), y_all(k:k+1, PG1repeat_all == commonrepeat(j)), z1_all(k:k+1, PG1repeat_all == commonrepeat(j)),...
                '-', 'Color', cmap(k, :), 'LineWidth', 1);
        end
        plot3(x_all(1, PG2repeat_all == commonrepeat(j)), y_all(1, PG1repeat_all == commonrepeat(j)), z1_all(1, PG1repeat_all == commonrepeat(j)),...
            'o', 'Color', cmap(1, :), 'MarkerSize', markersize, 'MarkerFaceColor', cmap(1, :));
        plot3(x_all(end, PG2repeat_all == commonrepeat(j)), y_all(end, PG1repeat_all == commonrepeat(j)), z1_all(end, PG1repeat_all == commonrepeat(j)),...
            's', 'Color', cmap(end, :), 'MarkerSize', markersize, 'MarkerFaceColor', cmap(end, :));
...
function PlotRelativeTrajectory_button_Callback(hObject, eventdata, handles)
clc;
DataDir = get(handles.dir_text, 'String');
ExpNumber = get(handles.Exp_list, 'Value');
SpotNumber = get(handles.Spot_list, 'Value');
...
PG2REF = pointsToWorld(CP2, CP2.RotationMatrices(:, :, end), CP2.TranslationVectors(end, :), PG2REF);
for i = 1:length(SpotNumber)
    pawl2nose_all = cell(0);
    pawr2nose_all = cell(0);
    pawl2pawr_all = cell(0);
    ...
    for j = 1:length(ExpNumber)
        ...
        for k = 1:length(RepeatNumber)
            ...
            try
                v.data = load([DataDir '\exp' num2str(ExpNumber(j), '%03d') '\PG2gpio' num2str(TrialNumber) '.csv']);
                startID = find(v.data(:, 1) == 1, 1, 'first');
                stopID = find(v.data(:, 1) == 1, 1, 'last');
                
                load([DataDir '\exp' num2str(ExpNumber(j), '%03d') '\PG2camera' num2str(TrialNumber) '_trajectory.mat']);
                x = trajectory(startID-FramesBL:stopID+FramesAL, 1);
                z2 = trajectory(startID-FramesBL:stopID+FramesAL, 2);
                temp = [x resolution(1)-z2];
                temp = undistortPoints(temp, CP2);
                temp = pointsToWorld(CP2, CP2.RotationMatrices(:, :, end), CP2.TranslationVectors(end, :), temp);
                x = (temp(:, 1)-PG2REF(1))*inch2mmcst;
                ...
                pawl2nose = vecnorm([x_pawl z2_pawl], 2, 2);
                pawr2nose = vecnorm([x_pawr z2_pawr], 2, 2);
                pawl2pawr = vecnorm([x_pawl-x_pawr z2_pawl-z2_pawr], 2, 2);
                pawl2nose_all(end+1) = {pawl2nose};
                pawr2nose_all(end+1) = {pawr2nose};
                pawl2pawr_all(end+1) = {pawl2pawr};
    ...
    onframes = nan(1, numel(pawl2nose_all));
    for j = 1:numel(pawl2nose_all)
        onframes(j) = numel(pawl2nose_all{j})-FramesBL-FramesAL;
    end
    pawl2nose = nan(FramesBL+max(onframes)+FramesAL, numel(onframes));
    pawr2nose = nan(FramesBL+max(onframes)+FramesAL, numel(onframes));
    pawl2pawr = nan(FramesBL+max(onframes)+FramesAL, numel(onframes));
    t = ((1:FramesBL+max(onframes)+FramesAL)-FramesBL-1)'/FrameRate;
    figure;
    hold on;
    for j = 1:numel(onframes)
        hpl(j) = plot(t(1:numel(pawl2nose_all{j})), pawl2nose_all{j}, '-', 'Color', [0 187 255]/255, 'LineWidth', 1);
        set(hpl(j), 'ButtonDownFcn', @(object, event)ShowRepeatNumber(object, event, PG2repeat_all(j)));
        hpr(j) = plot(t(1:numel(pawr2nose_all{j})), pawr2nose_all{j}, '-', 'Color', [0 255 0]/255, 'LineWidth', 1);
        set(hpr(j), 'ButtonDownFcn', @(object, event)ShowRepeatNumber(object, event, PG2repeat_all(j)));
        
        pawl2nose(1:numel(pawl2nose_all{j}), j) = pawl2nose_all{j};
        pawr2nose(1:numel(pawr2nose_all{j}), j) = pawr2nose_all{j};
    end
    ...
    xlabel('Time (s)');
    ylabel('Hand-to-nose distance (mm)');
    ...
    hold on;
    for j = 1:numel(onframes)
        hp(j) = plot(t(1:numel(pawl2pawr_all{j})), pawl2pawr_all{j}, '-', 'Color', [0.8 0.8 0.8], 'LineWidth', 1);
        set(hp(j), 'ButtonDownFcn', @(object, event)ShowRepeatNumber(object, event, PG2repeat_all(j)));
        
        pawl2pawr(1:numel(pawl2pawr_all{j}), j) = pawl2pawr_all{j};
    end
    ...
    set(gca, 'Children', [hc(2:end); hc(1)]);
    xlabel('Time (s)');
    ylabel('Hand-to-hand distance (mm)');
    
    figure;
    hold on;
    hp(1) = plot_tj_MeanSEM(t, pawl2nose, [0 0 255]/255, [0 0 255]/255, '', '', '');
    hp(2) = plot_tj_MeanSEM(t, pawr2nose, [0 130 0]/255, [0 130 0]/255, 'Time (s)', 'Hand-to-nose distance (mm)', '');
    yl = ylim;
    patch([0 0 mean(onframes)/FrameRate mean(onframes)/FrameRate], [0 yl(2) yl(2) 0], [0.9 0.9 0.9], 'EdgeColor', 'none');
    hc = get(gca, 'Children');
    set(gca, 'Children', [hc(2:end); hc(1)]);
    ...
    figure;
    hold on;
    plot_tj_MeanSEM(t, pawl2pawr, [255 153 0]/255, [255 153 0]/255, 'Time (s)', 'Hand-to-hand distance (mm)', '');
    yl = ylim;
    patch([0 0 mean(onframes)/FrameRate mean(onframes)/FrameRate], [0 yl(2) yl(2) 0], [0.9 0.9 0.9], 'EdgeColor', 'none');
    hc = get(gca, 'Children');
    set(gca, 'Children', [hc(2:end); hc(1)]);
