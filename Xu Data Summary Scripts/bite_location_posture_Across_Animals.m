...
% Version: 1.0
%*---------------------------------------------------------------------*
%% bite posture and location analysis across mice
curpwd = pwd;
try
   cd(pathname); 
end
...
bitelocation_NI = nan(3, 6, N);
blocation_NI = nan(3, 4, N);
side_NI = nan(N, 2);
...
ctop_I = nan(3, N, 3);
cbottom_I = nan(3, N, 3);
bitelocation_I = nan(3, 6, N);
blocation_I = nan(3, 4, N);
side_I = nan(N, 2);
...
%         ceyer_NI(:, i, 1) = mean(squeeze(result.ceyer_all_NI(trange+1, :, IDall)), 2, 'omitnan');
        ...
        bitelocation_NI(1, :, i) = result.bitelocation_NI;
        blocation_NI(1, :, i) = result.blocation_NI;
    end
    if numel(IDleft) > thr
...
%         ceyer_NI(:, i, 2) = mean(squeeze(result.ceyer_all_NI(trange+1, :, IDleft)), 2, 'omitnan');
        ...
        bitelocation_NI(2, :, i) = result.bitelocationL_NI;
        blocation_NI(2, :, i) = result.blocationL_NI;
...
%         ceyer_NI(:, i, 3) = mean(squeeze(result.ceyer_all_NI(trange+1, :, IDright)), 2, 'omitnan');
        ...
        bitelocation_NI(3, :, i) = result.bitelocationR_NI;
        blocation_NI(3, :, i) = result.blocationR_NI;
...
%             ceyer_I(:, i, 1) = mean(squeeze(result.ceyer_all_I(trange+1, :, IDall)), 2, 'omitnan');
            ...
            bitelocation_I(1, :, i) = result.bitelocation_I;
            blocation_I(1, :, i) = result.blocation_I;
        end
        if numel(IDleft) > thr
            ...
            bitelocation_I(2, :, i) = result.bitelocationL_I;
            blocation_I(2, :, i) = result.blocationL_I;
...
%             ceyer_I(:, i, 3) = mean(squeeze(result.ceyer_all_I(trange+1, :, IDright)), 2, 'omitnan');
            ...
            bitelocation_I(3, :, i) = result.bitelocationR_I;
            blocation_I(3, :, i) = result.blocationR_I;
...
figure;
disp('Bite location X to midline');
subplot(2, 3, 1);
paired_plot(abs(mean(squeeze(bitelocation_NI(1, [1 4], :)), 'omitnan')-xmidline),...
    abs(mean(squeeze(bitelocation_I(1, [1 4], :)), 'omitnan')-xmidline), 'Bite location X to midline (mm)', filename, 'dot');
disp('Bite location Y');
subplot(2, 3, 2);
paired_plot(squeeze(bitelocation_NI(1, 5, :)), squeeze(bitelocation_I(1, 5, :)), 'Bite location Y (mm)', filename, 'dot');
disp('Bite location Z');
subplot(2, 3, 3);
paired_plot(squeeze(bitelocation_NI(1, 2, :)), squeeze(bitelocation_I(1, 2, :)), 'Bite location Z (mm)', filename, 'dot');
disp('BL X to midline');
subplot(2, 3, 4);
paired_plot(abs(squeeze(blocation_NI(1, 1, :))-xmidline'), abs(squeeze(blocation_I(1, 1, :))-xmidline'), 'BL X to midline (mm)', filename, 'dot');
disp('BL Y');
subplot(2, 3, 5);
paired_plot(squeeze(blocation_NI(1, 2, :)), squeeze(blocation_I(1, 2, :)), 'BL Y (mm)', filename, 'dot');
disp('BL Z');
subplot(2, 3, 6);
paired_plot(squeeze(blocation_NI(1, 3, :)), squeeze(blocation_I(1, 3, :)), 'BL Z (mm)', filename, 'dot');

title_text = {'All', 'Left', 'Right'};
for i = 1:3
    temp = bitelocation_NI(i, [3 6], :);
    temp = squeeze(temp);
    temp = mean(temp, 'omitnan');
    rsquare_NI = temp;
    temp = bitelocation_I(i, [3 6], :);
    temp = squeeze(temp);
    temp = mean(temp, 'omitnan');
    rsquare_I = temp;
    disp('Rsquare average of front and top');
    figure;
    subplot(1, 2, 1);
    paired_plot(rsquare_NI, rsquare_I, 'Rsquare', filename, 'bar');
    title([title_text{i} ': average of front and top']);
    
    temp = blocation_NI(i, 4, :);
    temp = squeeze(temp);
    rsquare_NI = temp;
    temp = blocation_I(i, 4, :);
    temp = squeeze(temp);
    rsquare_I = temp;
    disp('Rsquare');
    subplot(1, 2, 2);
...
%     set(gcf, 'Name', title_text{i});
    ...
    bitelocation_NI(i, 1, :) = bitelocation_NI(i, 1, :)-reshape(ref(1, :), [1, 1, N]);
    bitelocation_NI(i, 4, :) = bitelocation_NI(i, 4, :)-reshape(ref(1, :), [1, 1, N]);
    bitelocation_NI(i, 2, :) = bitelocation_NI(i, 2, :)-reshape(ref(3, :), [1, 1, N]);
    bitelocation_NI(i, 5, :) = bitelocation_NI(i, 5, :)-reshape(ref(2, :), [1, 1, N]);
    blocation_NI(i, 1, :) = blocation_NI(i, 1, :)-reshape(ref(1, :), [1, 1, N]);
    blocation_NI(i, 2, :) = blocation_NI(i, 2, :)-reshape(ref(2, :), [1, 1, N]);
    blocation_NI(i, 3, :) = blocation_NI(i, 3, :)-reshape(ref(3, :), [1, 1, N]);
    ...
    bitelocation_I(i, 1, :) = bitelocation_I(i, 1, :)-reshape(ref(1, :), [1, 1, N]);
    bitelocation_I(i, 4, :) = bitelocation_I(i, 4, :)-reshape(ref(1, :), [1, 1, N]);
    bitelocation_I(i, 2, :) = bitelocation_I(i, 2, :)-reshape(ref(3, :), [1, 1, N]);
    bitelocation_I(i, 5, :) = bitelocation_I(i, 5, :)-reshape(ref(2, :), [1, 1, N]);
    blocation_I(i, 1, :) = blocation_I(i, 1, :)-reshape(ref(1, :), [1, 1, N]);
    blocation_I(i, 2, :) = blocation_I(i, 2, :)-reshape(ref(2, :), [1, 1, N]);
    blocation_I(i, 3, :) = blocation_I(i, 3, :)-reshape(ref(3, :), [1, 1, N]);
    ...
    draw_individual_posture(cnose_NI(:, :, i), ceyel_NI(:, :, i), ceyer_NI(:, :, i), cpawl_NI(:, :, i), cpawr_NI(:, :, i), ctop_NI(:, :, i), cbottom_NI(:, :, i), bitelocation_NI(i, :, :), blocation_NI(i, :, :),...
        cnose_I(:, :, i), ceyel_I(:, :, i), ceyer_I(:, :, i), cpawl_I(:, :, i), cpawr_I(:, :, i), ctop_I(:, :, i), cbottom_I(:, :, i), bitelocation_I(i, :, :), blocation_I(i, :, :), filename);
    set(gcf, 'Name', title_text{i});
end

cd(curpwd);

function draw_individual_posture(cnose_all_NI, ceyel_all_NI, ceyer_all_NI, cpawl_all_NI, cpawr_all_NI, ctop_all_NI, cbottom_all_NI, bitelocation_all_NI, blocation_all_NI,...
    cnose_all_I, ceyel_all_I, ceyer_all_I, cpawl_all_I, cpawr_all_I, ctop_all_I, cbottom_all_I, bitelocation_all_I, blocation_all_I, filename)
...
for j = 1:size(ceyel_all_NI, 2)
    ...
    plot(bitelocation_all_NI(1, 1, j), bitelocation_all_NI(1, 2, j), 'Marker', markers{j}, 'Color', [0.49 0.18 0.56], 'MarkerSize', mksize, 'ButtonDownFcn', @displayfilename, 'UserData', filename{j});
    plot(blocation_all_NI(1, 1, j), blocation_all_NI(1, 3, j), 'Marker', markers{j}, 'Color', [0 0 0], 'MarkerSize', mksize, 'ButtonDownFcn', @displayfilename, 'UserData', filename{j});
end
...
for j = 1:size(ceyel_all_I, 2)
    ...
    plot(bitelocation_all_I(1, 1, j), bitelocation_all_I(1, 2, j), 'Marker', markers{j}, 'Color', [1 0 1], 'MarkerSize', mksize, 'ButtonDownFcn', @displayfilename, 'UserData', filename{j});
    plot(blocation_all_I(1, 1, j), blocation_all_I(1, 3, j), 'Marker', markers{j}, 'Color', [0.8 0.8 0.8], 'MarkerSize', mksize, 'ButtonDownFcn', @displayfilename, 'UserData', filename{j});
end
...
for j = 1:size(ceyel_all_NI, 2)
    ...
    plot(bitelocation_all_NI(1, 4, j), bitelocation_all_NI(1, 5, j), 'Marker', markers{j}, 'Color', [0.49 0.18 0.56], 'MarkerSize', mksize, 'ButtonDownFcn', @displayfilename, 'UserData', filename{j});
    plot(blocation_all_NI(1, 1, j), blocation_all_NI(1, 2, j), 'Marker', markers{j}, 'Color', [0 0 0], 'MarkerSize', mksize, 'ButtonDownFcn', @displayfilename, 'UserData', filename{j});
end
...
for j = 1:size(ceyel_all_I, 2)
    ...
    plot(bitelocation_all_I(1, 4, j), bitelocation_all_I(1, 5, j), 'Marker', markers{j}, 'Color', [1 0 1], 'MarkerSize', mksize, 'ButtonDownFcn', @displayfilename, 'UserData', filename{j});
    plot(blocation_all_I(1, 1, j), blocation_all_I(1, 2, j), 'Marker', markers{j}, 'Color', [0.8 0.8 0.8], 'MarkerSize', mksize, 'ButtonDownFcn', @displayfilename, 'UserData', filename{j});
end
axis equal;
xlabel('X (mm)');
ylabel('Y (mm)');
