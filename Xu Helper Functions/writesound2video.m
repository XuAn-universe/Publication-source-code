function writesound2video(app, FrameRate, SampleRate, audiodata, csvdata, readerobj, num_track, signal_filtered, duration, label_table, label_trace, sqsize, dotsize, labelcolor, alphavalue, fpdata)
brightness = app.BrightnessEditField.Value;
framecount = csvdata(:, 2);
framecount = framecount-framecount(1)+1;
time_interval = 1/readerobj.FrameRate;
videoFReader = vision.VideoFileReader([readerobj.Path '\' readerobj.Name]);
videoFWriter = vision.VideoFileWriter([readerobj.Path '\Sound_' readerobj.Name],...
    ...
    if n >= duration(1)*readerobj.FrameRate
        ...
        if ~app.sOnTwiceButton.Value
            ...
            if ~isempty(audiodata.laser_starttime) && app.OptSessionCheckBox.Value
                if ((n/readerobj.FrameRate >= audiodata.laser_starttime(1) && n/readerobj.FrameRate <= audiodata.laser_endtime(1))...
                        || (n/readerobj.FrameRate >= audiodata.laser_starttime(2) && n/readerobj.FrameRate <= audiodata.laser_endtime(2)))
                    videoFrame = insertShape(videoFrame, 'FilledRectangle', [0 0 sqsize sqsize], 'Color', 'w', 'Opacity', 1);
                end
            end
        end
