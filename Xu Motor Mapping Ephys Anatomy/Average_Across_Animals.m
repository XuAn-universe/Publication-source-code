...
for i = 1:3
    ...
    end_mean_all = zeros(nspots, n)/0;
    end_std_all = zeros(nspots, n)/0;
    for j = 1:n
        eval(['[d_mean_all(:, j), pvalue_all(:, j), end_mean_all(:, j), end_std_all(:, j)] = takecareNaN(data' num2str(j) '.startpoints_all(:, :, i), data' num2str(j) '.endpoints_all(:, :, i));']);
%         d_mean_all(:, j) = d_mean_all(:, j)/max(abs(d_mean_all(:, j)));
    ...
    end_mean = mean(end_mean_all, 2);
    end_std = mean(end_std_all, 2);
    im(:, :, 1) = mat2gray(reshape(end_mean, data.rows, data.columns))*0.67;
    im(:, :, 3) = 1-mat2gray(reshape(end_std, data.rows, data.columns));
    figure;
    subplot(1, 2, 1);
...
%%
%3D trajectory plot
tj1 = zeros(n, size(data1.trajectory_all, 2));
tj2 = zeros(n, size(data1.trajectory_all, 2));
tj3 = zeros(n, size(data1.trajectory_all, 2));
figure('Color', [1 1 1]);
a3 = axes;
hold(a3, 'on');
figure('Color', [1 1 1]);
...
axy = axes;
hold(axy, 'on');
for i = 1:size(data1.trajectory_all, 3)
    for k = 1:n
        temp1 = [];
        temp2 = [];
        temp3 = [];
        eval(['temp = data' num2str(k) '.trajectory_all;']);
        for j = 1:size(temp, 1)
            if ~isnan(temp(j, 1, i, 1)) && ~isnan(temp(j, 1, i, 2)) && ~isnan(temp(j, 1, i, 3))
                temp1 = [temp1; temp(j, :, i, 1)-temp(j, 1, i, 1)];
                temp2 = [temp2; temp(j, :, i, 2)-temp(j, 1, i, 2)];
                temp3 = [temp3; temp(j, :, i, 3)-temp(j, 1, i, 3)];
            end
        end
        tj1(k, :) = mean(temp1);
        tj2(k, :) = mean(temp2);
    ...
    plot3(a3, mean(tj1), mean(tj2), mean(tj3), '-', 'Color',facecolor_all(i, :), 'LineWidth', 2);
    plot3(a3, mean(tj1(:, end)), mean(tj2(:, end)), mean(tj3(:, end)), 's', 'Color', facecolor_all(i, :), 'MarkerSize', 8, 'MarkerFaceColor', facecolor_all(i, :));
    plot(axz, mean(tj1), mean(tj3), '-', 'Color',facecolor_all(i, :), 'LineWidth', 2);
    plot(axz, mean(tj1(:, end)), mean(tj3(:, end)), 's', 'Color', facecolor_all(i, :), 'MarkerSize', 8, 'MarkerFaceColor', facecolor_all(i, :));
    plot(ayz, mean(tj2), mean(tj3), '-', 'Color',facecolor_all(i, :), 'LineWidth', 2);
    plot(ayz, mean(tj2(:, end)), mean(tj3(:, end)), 's', 'Color', facecolor_all(i, :), 'MarkerSize', 8, 'MarkerFaceColor', facecolor_all(i, :));
    plot(axy, mean(tj1), mean(tj2), '-', 'Color',facecolor_all(i, :), 'LineWidth', 2);
    plot(axy, mean(tj1(:, end)), mean(tj2(:, end)), 's', 'Color', facecolor_all(i, :), 'MarkerSize', 8, 'MarkerFaceColor', facecolor_all(i, :));
end
xlabel(a3, 'dX (mm)');
ylabel(a3, 'dY (mm)');
zlabel(a3, 'dZ (mm)');
